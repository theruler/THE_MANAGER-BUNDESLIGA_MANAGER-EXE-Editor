...
from font_safety import (
    FontSafetyError, atomic_save_bytes, change_glyph_width,
    copy_glyph_payload, export_font_bytes, import_font_bytes,
    load_font_model, paste_glyph_payload, stage_font_apply,
...
class EXEFontEditor:
    ...
    def __init__(self, parent, get_exe_data, on_charmap_changed=None,
                 on_font_state_changed=None, can_change_charmap=None,
                 on_open_charmap=None, translate=None):
        self.parent               = parent
        ...
        if self.is_supported and self.current_font_key:
            if hasattr(self, "charmap_title_label"):
                self.charmap_title_label.config(
                    text=self._tr("font.charmap_title_for", font=self.current_font_key))
        elif hasattr(self, "charmap_title_label"):
            self.charmap_title_label.config(text=self._tr("font.charmap_title"))
        if not self.is_supported and hasattr(self, "font_status"):
            self.font_status.config(text=self._tr("font.load_first"))
    ...
    def _build_ui(self, parent):
        ...
        self.font_status = ttk.Label(bar, text=self._tr("font.load_first"),
        ...
        self.char_list = tk.Listbox(lf, width=22, font=("Consolas", 10),
        ...
        self.canvas = tk.Canvas(gf, bg="#1E1E1E", highlightthickness=0,
                                 width=8 * self.CELL, height=8 * self.CELL)
        self.canvas.pack(anchor=tk.CENTER, expand=True)
        self.canvas.bind("<Button-1>",
            lambda e: setattr(self, '_last_drag_state', self._toggle_pixel(e.x, e.y)))
        self.canvas.bind("<B1-Motion>",
            lambda e: self._last_drag_state is not None and
                      self._toggle_pixel(e.x, e.y, force=self._last_drag_state))
        ...
        self.width_spin = ttk.Spinbox(prop_group, from_=1, to=24,
        ...
        cpf = ttk.Frame(tools_group); cpf.pack(fill=tk.X, pady=(0, 2))
        self._reg(ttk.Button(cpf, command=self._copy_glyph),
                  "font.copy").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        self._reg(ttk.Button(cpf, command=self._paste_glyph),
                  "font.paste").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(2, 0))
        cif = ttk.Frame(tools_group); cif.pack(fill=tk.X, pady=2)
        self._reg(ttk.Button(cif, command=lambda: self._edit_char("clear")),
                  "font.clear").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        self._reg(ttk.Button(cif, command=lambda: self._edit_char("invert")),
                  "font.invert").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(2, 0))
        sf = ttk.Frame(tools_group); sf.pack(fill=tk.X, pady=(4, 2))
        for text, row, col, dir_ in [("▲", 0, 1, "up"), ("◀", 1, 0, "left"),
                                      ("▶", 1, 2, "right"), ("▼", 2, 1, "down")]:
            ttk.Button(sf, text=text, width=3,
                       command=lambda d=dir_: self._edit_char("shift", d)).grid(
        ...
        ief = ttk.Frame(file_group); ief.pack(fill=tk.X)
        self._reg(ttk.Button(ief, command=self._import_font),
                  "font.import").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        self._reg(ttk.Button(ief, command=self._export_font),
        ...
        self.alias_label = ttk.Label(rf, text="", font=("Segoe UI", 8, "italic"),
                                      foreground="#E67E22", wraplength=150)
    ...
    def build_charmap_panel(self, parent):
        ...
        self.charmap_title_label = ttk.Label(panel, text=self._tr("font.charmap_title"),
                                              font=("Segoe UI", 10, "bold"))
        self.charmap_title_label.pack(anchor=tk.W, pady=(0, 4))
        map_row = ttk.Frame(panel); map_row.pack(fill=tk.X, pady=(4, 2))
        self.map_byte_label = ttk.Label(map_row, text="0x00 →", font=("Consolas", 10))
        self.map_byte_label.pack(side=tk.LEFT, padx=(0, 4))
        ttk.Entry(map_row, textvariable=self.map_char_var, width=4,
                  font=("Consolas", 10)).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(map_row, text="+", width=2,
                   command=self._add_charmap_entry).pack(side=tk.LEFT)
        self.charmap_list = tk.Listbox(panel, width=22, height=14,
    ...
    def set_enabled(self, enabled: bool):
        ...
        def apply_state(widget):
            for child in widget.winfo_children():
                ...
                elif isinstance(child, (ttk.Button, ttk.Spinbox, ttk.Entry,
                                        tk.Listbox, tk.Canvas)):
    ...
    def _notify_font_state(self):
        if callable(self.on_font_state_changed):
            self.on_font_state_changed({
                "valid": bool(self.font_valid), "pending": bool(self.font_pending),
                "changed": bool(self.font_changed), "errors": list(self.font_errors),
            })

    def _set_font_state(self, *, valid=None, pending=None, changed=None, error=None):
        if valid   is not None: self.font_valid   = bool(valid)
        if pending is not None: self.font_pending  = bool(pending)
        if changed is not None: self.font_changed  = bool(changed)
    ...
    def _font_failure(self, error, *, show_dialog=True):
        ...
        if hasattr(self, "font_status"):
            self.font_status.config(text=self._tr("font.blocked_status", error=detail))
        if show_dialog:
            messagebox.showerror(self._tr("font.blocked_title"), detail)
        return False

    def mark_saved(self):
        if self.is_supported and self.raw_data:
            self.initial_raw_data = bytearray(self.raw_data)
            self._set_font_state(valid=self.font_valid, pending=self.font_pending,
                                  changed=False,
    ...
    def validate_live_state(self):
        if not self.is_supported or not self.game_profile or not self.raw_data:
            return self._font_failure(tr("font.err.live_unavailable"), show_dialog=False)
        main_data = self.get_exe_data()
        if not isinstance(main_data, bytearray) or not main_data:
            return self._font_failure(tr("font.err.buffer_unavailable"), show_dialog=False)
        try:
            ...
            for desc in self.game_profile["fonts"].values():
                for start_key, end_key in (("ptr_start", "ptr_end"),
                                            ("glyph_start", "glyph_end")):
                    start, end = desc[start_key], desc[end_key]
                    if self.raw_data[start:end] != main_data[start:end]:
                        raise FontSafetyError(
                            tr("fsafe.err.buffers_differ", start=start, end=end))
            self._set_font_state(valid=True, pending=self.font_pending,
    ...
    def reset_state(self, status=None):
        status = self._tr("font.load_first") if status is None else status
        self.is_supported = False; self.raw_data = bytearray()
        self.initial_raw_data = bytearray(); self.game_name = None
        self.game_profile = None; self.current_font_key = None
        self.font_data = []; self.font_valid = False; self.font_pending = False
        self.font_changed = False; self.font_errors = []; self.selected_idx = 0
        self._last_drag_state = None; self._clipboard = None
        if hasattr(self, "font_combo"):
            self.font_combo.config(values=[]); self.font_var.set("")
        if hasattr(self, "char_list"):  self.char_list.delete(0, tk.END)
        if hasattr(self, "canvas"):     self.canvas.delete("all")
        if hasattr(self, "alias_label"): self.alias_label.config(text="")
        if hasattr(self, "font_status"): self.font_status.config(text=status)
        if hasattr(self, "map_byte_label"): self.map_byte_label.config(text="0x00 →")
        if hasattr(self, "map_byte_var"):   self.map_byte_var.set("")
        if hasattr(self, "map_char_var"):   self.map_char_var.set("")
        if hasattr(self, "charmap_list"):   self.charmap_list.delete(0, tk.END)
    ...
    def load_from_raw(self, raw: bytearray, game_name: str):
        ...
        if not game_profile:
            self.font_status.config(text=self._tr("font.no_profile", profile=game_name))
            self._set_font_state(valid=False, error=tr("font.no_profile", profile=game_name))
            return False
        try:
            validate_all_fonts(raw, game_profile)
            self.raw_data = bytearray(raw); self.initial_raw_data = bytearray(raw)
            self.game_name = game_name; self.game_profile = game_profile
            self.is_supported = True
            font_names = list(self.game_profile["fonts"].keys())
            self.font_combo.config(values=font_names); self.font_combo.set(font_names[0])
            self.set_enabled(True)
            if not self._load_current_font(font_names[0]):
                raise FontSafetyError(tr("font.err.initial_load"))
            self._set_font_state(valid=True, pending=False, changed=False)
            return True
        except (FontSafetyError, KeyError, IndexError, tk.TclError) as exc:
            self.is_supported = False; self.set_enabled(False)
    ...
    def _load_current_font(self, font_key: str):
        ...
        try:
            ...
            return self._font_failure(exc)
        self.current_font_key = font_key; self.font_data = font_data; self.selected_idx = 0
        max_font_width = 24 if desc["type"] == "dynamic" else 8
        self.width_spin.config(to=max_font_width)
        self._refresh_list()
        self.char_list.selection_clear(0, tk.END); self.char_list.selection_set(0)
        self._on_char_select(None)
        self.font_status.config(text=self._tr("font.summary", font=font_key,
                                               chars=summary["pointer_count"],
                                               unique=summary["unique_count"]))
        self._refresh_charmap_list()
        self._set_font_state(valid=True, pending=False,
                              changed=self.raw_data != self.initial_raw_data)
        return True
    ...
    def _refresh_list(self):
        ...
        for ch in self.font_data:
            ...
            alias    = ch["alias_of"]
            label    = (f"0x{ac:02X} '{char_str}'  ≡ 0x{self.font_data[alias]['ascii']:02X}"
                        if alias is not None else f"0x{ac:02X} ({ac:3d})  '{char_str}'")
            self.char_list.insert(tk.END, label)
            if alias is not None:
                idx = self.char_list.size() - 1
                if idx >= 0:
                    self.char_list.itemconfig(idx, foreground="#999999")
    ...
    def _draw_grid(self):
        ...
        rows  = desc["rows"]; bpr = glyph["bytes_per_row"]; cols = bpr * 8
        width_px = glyph["width"]; cs = self.CELL
        self.canvas.config(width=cols * cs, height=rows * cs)
        self.canvas.delete("all")
        for r in range(rows):
            row_val = glyph["bitmap"][r] if r < len(glyph["bitmap"]) else 0
            for c in range(cols):
                bit_idx = cols - 1 - c; is_on = bool(row_val & (1 << bit_idx))
                fill = (("#1F618D" if is_on else "#1A252F") if c >= width_px
                        else ("#3498DB" if is_on else "#2C3E50"))
                x1, y1 = c * cs, r * cs
                self.canvas.create_rectangle(x1, y1, x1 + cs, y1 + cs,
                                              fill=fill, outline="#111111")
        self.canvas.create_line(width_px * cs, 0, width_px * cs, rows * cs,
                                 fill="#E74C3C", width=2, dash=(4, 4))

    def _on_font_select(self, event=None):
        if not self.is_supported: return
        requested = self.font_var.get(); previous = self.current_font_key
        if requested == previous: return
        if self.font_pending:
            decision = messagebox.askyesnocancel(
                self._tr("font.pending_title"),
                self._tr("font.pending_msg", font=previous))
            if decision is None:
                self.font_var.set(previous); return
    ...
    def _on_char_select(self, event=None):
        ...
        ch    = self.font_data[self.selected_idx]
        glyph = self._glyph(ch)
        self.width_var.set(glyph["width"]); self.width_spin.config(to=glyph["max_width"])
        alias = ch["alias_of"]
        if alias is not None:
            alias_ac = self.font_data[alias]["ascii"]
            self.alias_label.config(text=self._tr("font.alias", code=alias_ac, char=chr(alias_ac)))
        else:
            self.alias_label.config(text="")
        byte_val = ch["ascii"]; self.map_byte_var.set(f"{byte_val:02x}")
    ...
    def _on_width_change(self, event=None):
        ...
        try:
            ...
            old_width, old_bpr, old_bitmap = glyph["width"], glyph["bytes_per_row"], list(glyph["bitmap"])
            w = self.width_var.get()
            change_glyph_width(glyph, desc, w)
            if not self._refresh_pending_state():
                glyph["width"] = old_width; glyph["bytes_per_row"] = old_bpr
                glyph["bitmap"] = old_bitmap; self.width_var.set(old_width); return
            self._draw_grid()
    ...
    def _toggle_pixel(self, x, y, force=None):
        ...
        glyph = self._glyph(self.font_data[self.selected_idx])
        desc  = self.game_profile["fonts"][self.current_font_key]
        cols  = glyph["bytes_per_row"] * 8; cs = self.CELL
        c, r  = x // cs, y // cs
        ...
        if not self._refresh_pending_state():
            glyph["bitmap"][r] = old_value; return None
        self._draw_grid(); return new_state
    ...
    def _edit_char(self, action, direction=None):
        ...
        if not self._refresh_pending_state():
            glyph["bitmap"] = old_bitmap; return
        self._draw_grid()

    def _export_font(self):
        if not self.is_supported or not self.font_data or not self.current_font_key:
            messagebox.showwarning(self._tr("font.no_data"), self._tr("font.no_data_msg")); return
        desc     = self.game_profile["fonts"][self.current_font_key]
        filepath = filedialog.asksaveasfilename(
            title=self._tr("font.fd_export"), initialfile=self.current_font_key,
        ...
        try:
            ...
            messagebox.showerror(self._tr("font.export_error"), str(exc)); return
        self.font_status.config(text=self._tr("font.exported", font=self.current_font_key,
    ...
    def _import_font(self):
        ...
        filepath = filedialog.askopenfilename(
            title=self._tr("font.fd_import", font=self.current_font_key),
            filetypes=[("Font files", "*.FON *.fon *.bin"), ("All Files", "*.*")])
        if not filepath: return
        try:
            ...
            self._font_failure(tr("font.err.import_failed", error=exc)); return
        self.font_data = staged
        self._refresh_pending_state(); self._refresh_list()
        self.char_list.selection_clear(0, tk.END)
        self.char_list.selection_set(self.selected_idx)
        self._on_char_select(None)
        self.font_status.config(text=self._tr("font.imported",
                                               n=len(unique_glyphs(staged)),
                                               file=os.path.basename(filepath),
    ...
    def _parse_charmap_byte(value: str) -> str:
        ...
        if text.startswith(("0x", "0X")): text = text[2:]
    ...
    def _refresh_ui(self, what="charmap"):
        if what == "charmap": self._refresh_charmap_list()
        if hasattr(self, "map_byte_label"): self.map_byte_label.config(text="0x00 →")
        self.map_byte_var.set(""); self.map_char_var.set("")
    ...
    def _add_charmap_entry(self):
        ...
        if not uni_char:
            messagebox.showerror(self._tr("font.invalid_entry"),
                                  self._tr("font.invalid_entry_msg")); return
        try:
            byte_str = self._parse_charmap_byte(self.map_byte_var.get())
        except ValueError as exc:
            messagebox.showerror(self._tr("font.invalid_byte"), str(exc)); return
        if not self._charmap_change_allowed(): return
        self._font_charmap()[byte_str] = uni_char[0]
        save_config(self.cfg); self._refresh_ui(); self._notify_charmap_changed()
    ...
    def _on_charmap_select(self, event=None):
        ...
        entry = self.charmap_list.get(sel[0])
        char_part = entry.split("→")[1].strip() if "→" in entry else ""
    ...
    def _notify_charmap_changed(self):
        if callable(self.on_charmap_changed):
            self.on_charmap_changed(self.game_name, self.current_font_key,
                                     dict(self._font_charmap()))

    def _write_to_exe(self, *, show_success=True):
        if not self.is_supported or not self.raw_data or not self.game_profile:
            messagebox.showwarning(self._tr("font.no_data"), self._tr("font.no_exe_msg"))
            return False
        main_data = self.get_exe_data()
        if not isinstance(main_data, bytearray) or not main_data:
            return self._font_failure(tr("font.err.buffer_unavailable"))
        old_raw  = bytearray(self.raw_data); old_main = bytearray(main_data)
        selected = self.selected_idx; committed = False
        try:
            ...
            if raw_result["changed_offsets"] != main_result["changed_offsets"]:
                raise FontSafetyError(tr("fsafe.err.buffers_state"))
            committed = True
            self.raw_data[:] = staged_raw; main_data[:] = staged_main
            ...
            if committed:
                self.raw_data[:] = old_raw; main_data[:] = old_main
                try:
                    self.font_data = load_font_model(
                        self.raw_data, self.game_profile, self.current_font_key)
                    self.selected_idx = min(selected, len(self.font_data) - 1)
                    ...
                    self.font_data = []
            return self._font_failure(tr("font.err.apply_failed", error=exc))
        written = raw_result["unique_count"]
        self.font_status.config(text=self._tr("font.written", n=written,
                                               font=self.current_font_key))
        self._refresh_list()
        self.char_list.selection_clear(0, tk.END)
        self.char_list.selection_set(self.selected_idx)
        self._on_char_select(None)
        if show_success:
            messagebox.showinfo(self._tr("font.written_title"),
                                 self._tr("font.written_msg", n=written))
        return True

    def _copy_glyph(self):
        if not self.is_supported or not self.font_data or self.selected_idx >= len(self.font_data):
            messagebox.showwarning(self._tr("font.no_selection"),
                                    self._tr("font.no_selection_msg")); return
        ch   = self.font_data[self.selected_idx]
        desc = self.game_profile["fonts"][self.current_font_key]
        self._clipboard = copy_glyph_payload(self._glyph(ch), desc)
        ac = ch["ascii"]; char_str = chr(ac) if 0x20 <= ac <= 0x7E else "·"
        self.font_status.config(text=self._tr("font.copied", code=ac, char=char_str))

    def _paste_glyph(self):
        if not self.is_supported: return
        if self._clipboard is None:
            messagebox.showwarning(self._tr("font.empty_clipboard"),
                                    self._tr("font.empty_clipboard_msg")); return
        ...
        try:
            ...
            self._font_failure(tr("font.err.paste_rejected", error=exc)); return
        self.width_var.set(glyph["width"]); self._draw_grid()
        ac = ch["ascii"]; char_str = chr(ac) if 0x20 <= ac <= 0x7E else "·"
        self.font_status.config(text=self._tr("font.pasted", code=ac, char=char_str))
