...
def _report_config_problem(message: str) -> None:
    try:
        ...
        messagebox.showwarning(tr("utils.cfg.title"), message)
    except Exception:
...
def _lock_config(reason: str) -> dict:
    ...
    _report_config_problem(tr("utils.cfg.damaged", reason=reason))
    return {}
...
def save_config(cfg: dict) -> None:
    if _CONFIG_LOCKED:
        _report_config_problem(
            tr("utils.cfg.locked", reason=_CONFIG_LOCK_REASON or "")
        )
        return
    try:
        ...
        _report_config_problem(tr("utils.cfg.save_failed", error=e))
...
__all__ = [
    'load_config', 'save_config', 'get_default_config', 'get_game_config',
    'CONFIG_FILE', 'DATA_DIR', 'DEEPL_KEY_FILE', 'DEFAULT_PROFILE_NAME',
]
