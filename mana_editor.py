...
class _LeagueTab(ttk.Frame):
    ...
    def _show_team(self, ti: int):
        ...
        stat = ttk.LabelFrame(right, text="Previous season statistics", padding=(8, 6))
        ...
        ttk.Label(rows_f, text="scored", foreground=MUTED, font=("Segoe UI", 7)).grid(row=2, column=1, sticky="e", padx=(0, 2), pady=3)
        gls_max_fe.grid(row=2, column=2, sticky="w", pady=3)
        w["gls"].append(gls_max_fe)
        gls_min_fe = _FixedEntry(rows_f, 0, 254, width=4, on_change=lambda: self._writeback(ti))
        gls_min_fe.set_int(t["gls"][1])
        ttk.Label(rows_f, text="conc.", foreground=MUTED, font=("Segoe UI", 7)).grid(row=2, column=3, sticky="e", padx=(4, 2), pady=3)
