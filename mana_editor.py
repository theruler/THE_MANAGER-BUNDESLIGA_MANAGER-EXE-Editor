...
LEAGUE_BOUNDS = [(0, 18, 18), (18, 20, 20), (38, 20, 20), (58, 6, 6)]
...
def parse_mana(data: bytes) -> dict:
    ...
    ctfl = [(data[HDR_CTF_L_OFF+i*3], data[HDR_CTF_L_OFF+i*3+1], data[HDR_CTF_L_OFF+i*3+2]) for i in range(N_LEAGUE)]
    ctfe = [(data[HDR_CTF_E_OFF+i*3], data[HDR_CTF_E_OFF+i*3+1], data[HDR_CTF_E_OFF+i*3+2]) for i in range(N_EUROPE)]
    ...
    for i in range(N_LEAGUE):
        ...
        teams.append({"name": _dec(data[base: base+23]), "players": players, "logo": logo[i], "ctf": list(ctfl[i]), "pts": list(pts[i]), "gls": list(gls[i]), "rank": rnk[i]})
    europe = []
...
class _FixedEntry(tk.Entry):
    def __init__(self, parent, lo: int, hi, on_change=None, width=3, **kw):
        ...
        super().__init__(parent, textvariable=self._var, width=width, justify="center", font=("Consolas", 9), **kw)
        ...
        if on_change:
            self._var.trace_add("write", self._traced_change)
    ...
    def _on_key_press(self, event):
        if event.char.isdigit():
            try:
                ...
                pass
        elif event.char and not event.char.isdigit() and ord(event.char) >= 32:
            if not (event.state & 0x4):
                return "break"

    def _traced_change(self, *args):
        val = self._var.get()
        if val == "":
            return
        if val.isdigit() and self._on_change and not getattr(self, '_updating', False):
            self._on_change()

    def _on_focus_in(self, event):
        self.after_idle(lambda: (self.select_range(0, "end"), self.icursor("end")))

    def _on_focus_out(self, event):
        val = self._var.get()
        if not val.isdigit():
            self.set_int(self._lo)
        else:
            self.set_int(int(val))
    ...
    def set_int(self, v: int):
        self._updating = True
        self._var.set(str(_clamp(v, self._lo, self._get_hi())))
...
class _LeagueTab(ttk.Frame):
    ...
    def _build_column(self, li: int, first: int, count: int, rank_max: int, col_offset: int):
        ...
        for row_i in range(count):
            ...
            rank_fe = _FixedEntry(self._grid_frame, 0, rank_max, width=3, on_change=lambda idx=ti: self._rank_changed(idx))
            rank_fe.set_int(t["rank"])
            rank_fe.config(font=("Consolas", 10), relief="solid", bg="#e8ecf0", highlightthickness=0)
    ...
    def _rank_changed(self, ti: int):
        ...
        if fe:
            self._parsed["teams"][ti]["rank"] = fe.get_int()
        w = self._widgets.get(ti)
        if w and "rank" in w and not self._building:
            w["rank"].set_int(self._parsed["teams"][ti]["rank"])
        if self._on_dirty:
            self._on_dirty()

    def _sort_by_rank(self, li: int, first: int, count: int):
        rank_max = LEAGUE_BOUNDS[li][2]
        base_col = li * 2
        for widget in self._grid_frame.grid_slaves():
            info = widget.grid_info()
            if info and "column" in info and info["column"] in (base_col, base_col + 1):
                widget.destroy()
        for ti in range(first, first + count):
            self._team_btns.pop(ti, None)
            self._rank_entries.pop(ti, None)

        teams = self._parsed["teams"]
        sel_team = teams[self._cur_team] if self._cur_team is not None else None
        group = teams[first:first + count]
        group.sort(key=lambda t: t["rank"])
        teams[first:first + count] = group
        if sel_team in group:
    ...
    def _swap_teams(self, a: int, b: int):
        for child in self._detail_outer.winfo_children():
            child.destroy()
        self._widgets.clear()
    ...
    def _show_placeholder(self):
        ...
        self._widgets.clear()
        self._logo_cvs.clear()
        ttk.Label(self._detail_outer, text="← Click on a team to edit it", foreground=MUTED, font=("Segoe UI", 12, "italic")).pack(expand=True)
    ...
    def _show_team(self, ti: int):
        ...
        def _limit_name(*_, nv=name_var):
            ...
            if len(val) > 20:
                nv.set(val[:20])
        name_ent = ttk.Entry(name_lf, textvariable=name_var, width=20, font=("Consolas", 10))
        ...
        for p in range(20):
            ...
            tk.Label(row_f, text=role, width=2, anchor="center", bg=rclr, fg="white", font=("Segoe UI", 7, "bold"), relief="flat").pack(side="left", padx=(0, 2))
            pvar = tk.StringVar(value=t["players"][p])
            def _limit_player(*_, pv=pvar):
                val = pv.get()
                if len(val) > 13:
                    pv.set(val[:13])
            ttk.Entry(row_f, textvariable=pvar, width=18, font=("Consolas", 9)).pack(side="left", fill="x", expand=True)
        ...
        stat = ttk.LabelFrame(right, text="Statistics", padding=(8, 6))
        stat.pack(fill="x", pady=(0, 8))
        for j in range(3):
            stat.columnconfigure(j, weight=1)
        for j, lbl in enumerate(("CON", "TEC", "FOR")):
            ttk.Label(stat, text=lbl, anchor="center", font=("Segoe UI", 7, "bold"), foreground=ACCENT).grid(row=0, column=j, sticky="ew", padx=3, pady=(0, 2))
        w["ctf"] = []
        for j in range(3):
            fe = _FixedEntry(stat, 0, 99, width=3, on_change=lambda: self._writeback(ti))
            fe.set_int(t["ctf"][j])
            fe.grid(row=1, column=j, padx=3, pady=(0, 4))
            w["ctf"].append(fe)
        ttk.Separator(stat, orient="horizontal").grid(row=2, column=0, columnspan=3, sticky="ew", pady=(2, 8))
        rows_f = ttk.Frame(stat)
        rows_f.grid(row=3, column=0, columnspan=3, sticky="ew")
        rows_f.columnconfigure(0, weight=0, minsize=60)
        rows_f.columnconfigure(1, weight=0)
        rows_f.columnconfigure(2, weight=0)
        rows_f.columnconfigure(3, weight=0)
        rows_f.columnconfigure(4, weight=0)
        LABEL_W = 7
        ttk.Label(rows_f, text="RANK", width=LABEL_W, anchor="e", foreground=MUTED, font=("Segoe UI", 9, "bold")).grid(row=0, column=0, sticky="e", padx=(0, 4), pady=3)
        ttk.Label(rows_f, text=f"(0-{rank_max})", foreground=MUTED, font=("Segoe UI", 8)).grid(row=0, column=1, sticky="e", padx=(0, 2), pady=3)
        rank_fe = _FixedEntry(rows_f, 0, rank_max, width=4, on_change=lambda: self._writeback(ti))
        rank_fe.set_int(t["rank"])
        rank_fe.grid(row=0, column=2, sticky="w", pady=3)
        w["rank"] = rank_fe
        ...
        ttk.Label(rows_f, text="max", foreground=MUTED, font=("Segoe UI", 8)).grid(row=1, column=1, sticky="e", padx=(0, 2), pady=3)
        pts_max_fe.grid(row=1, column=2, sticky="w", pady=3)
        w["pts"].append(pts_max_fe)
        pts_min_fe = _FixedEntry(rows_f, 0, lambda: w["pts"][0].get_int(), width=4, on_change=lambda: self._writeback(ti))
        pts_min_fe.set_int(t["pts"][1])
        ttk.Label(rows_f, text="min", foreground=MUTED, font=("Segoe UI", 8)).grid(row=1, column=3, sticky="e", padx=(4, 2), pady=3)
        ...
        gls_max_fe.set_int(t["gls"][0])
        ttk.Label(rows_f, text="max", foreground=MUTED, font=("Segoe UI", 8)).grid(row=2, column=1, sticky="e", padx=(0, 2), pady=3)
        gls_max_fe.grid(row=2, column=2, sticky="w", pady=3)
        w["gls"].append(gls_max_fe)
        gls_min_fe = _FixedEntry(rows_f, 0, lambda: w["gls"][0].get_int(), width=4, on_change=lambda: self._writeback(ti))
        gls_min_fe.set_int(t["gls"][1])
        ttk.Label(rows_f, text="min", foreground=MUTED, font=("Segoe UI", 8)).grid(row=2, column=3, sticky="e", padx=(4, 2), pady=3)
        gls_min_fe.grid(row=2, column=4, sticky="w", pady=3)
        w["gls"].append(gls_min_fe)
        ...
        logo_sb = ttk.Spinbox(logo_lf, from_=0, to=255, width=6, textvariable=logo_var, justify="center", command=lambda _ti=ti: self._on_logo_id_change(_ti))
    ...
    def _draw_logo_display(self, cvs: tk.Canvas, logo_id: int):
        ...
        possible_paths = [os.path.join(pic_dir, f"{logo_id:02d}.VGA"), os.path.join(pic_dir, f"{logo_id}.VGA"), os.path.join(pic_dir, f"{logo_id:02d}.vga"), os.path.join(pic_dir, f"{logo_id}.vga")]
        vga_path = None
...
class _UefaTab(ttk.Frame):
    ...
    def _build(self):
        ...
        for col in range(NCOLS):
            bc = col * 6
            ttk.Label(outer, text="#", width=3, anchor="e", foreground=MUTED, font=("Segoe UI", 7, "bold")).grid(row=0, column=bc, padx=(2, 1))
            ttk.Label(outer, text="TEAM NAME", width=20, anchor="w", font=("Segoe UI", 8, "bold")).grid(row=0, column=bc+1, padx=(0, 2))
            for j, lbl in enumerate(("C", "T", "F")):
                ttk.Label(outer, text=lbl, width=3, anchor="center", font=("Segoe UI", 8, "bold"), foreground=ACCENT).grid(row=0, column=bc+2+j, padx=1)
            if col < NCOLS - 1:
                sep_c = tk.Canvas(outer, width=3, bg="#aab4be", highlightthickness=0)
                sep_c.grid(row=0, column=bc+5, rowspan=rows_per_col + 2, sticky="ns", padx=(3, 3))
        sep_hdr = tk.Canvas(outer, height=3, bg="#aab4be", highlightthickness=0)
        sep_hdr.grid(row=1, column=0, columnspan=NCOLS * 6, sticky="ew", pady=(1, 2))
